...
const BALANCE={baseHp:100,hpPerWave:.04,hpGrowth:1.028,earlyEaseEnd:40,earlyBlendEnd:55,earlyHpStart:.65,earlyHpEnd:.8,hardRampStart:40,hardRampMax:1.8,baseSpeed:.76,speedPerWave:.003,maxSpeed:1.55,spawnInterval:600};
...
const spawnIntervalForWave=_wave=>BALANCE.spawnInterval;
const goldPerKill=wave=>wave<=40?1:wave<=70?2:wave<=90?3:4;

const naturalSaintHands=13*(choose(4,3)*choose(48,2)+2*choose(48,2));
...
const units=[
  ...
  ["궁수",15.6,.5,"치명타 50%·5배"],["연금술사",17.28,.5,"장판"],["사제",21.92,.65,"아군 버프"],["왕실기사",43.5,1.15,"보스 추가 피해"],
...
console.log("\n[성녀 자연 확률 · 천장 보정 없음]");
console.table([{natural:`${(naturalSaintRate*100).toFixed(4)}%`,averageHands:(1/naturalSaintRate).toFixed(1)}]);
