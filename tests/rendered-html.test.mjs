...
test("현재 전투·연출·모바일 규칙을 고정한다", async () => {
  ...
  assert.match(page, /\{selectedRerollCount \+ 1\}\)/);
  ...
  assert.match(page, /NORMAL_HP_DIFFICULTY_STEPS = \[\[10, 2\], \[20, 2\.2\], \[30, 3\.3\], \[40, 3\.9\], \[50, 4\.7\], \[60, 5\.5\], \[70, 6\.15\], \[80, 7\], \[90, 8\], \[100, 9\]\]/);
  assert.match(page, /BOSS_HP_BY_WAVE: Record<number, number> = \{ 10: 45000, 20: 80000, 30: 140000, 40: 220000, 50: 340000, 60: 500000, 70: 720000, 80: 1000000, 90: 1550000, 100: 2600000 \}/);
  ...
  assert.deepEqual(bossHpValues, [45000, 80000, 140000, 220000, 340000, 500000, 720000, 1000000, 1550000, 2600000]);
  assert.ok(bossHpValues.every((hp, index) => index === 0 || hp > bossHpValues[index - 1]), "보스 체력은 매 보스 웨이브마다 증가해야 합니다");
  assert.equal(Math.ceil(bossHpValues.at(-1) / 180), 14445);
  assert.match(page, /function hpDifficultyForWave\(wave: number\)/);
  assert.match(page, /function bossHpForWave\(wave: number\) \{ return BOSS_HP_BY_WAVE\[wave\] \?\? BOSS_HP_BY_WAVE\[100\]; \}/);
  ...
  assert.match(page, /공격 사거리 4칸 · 버프 범위 3칸 내 사제 외 아군 공격력 \+20% · 공격속도 \+20% · 사제 수만큼 중복/);
  ...
  assert.match(page, /return \{ damage: stackCount \* \.2, speed: stackCount \* \.2 \}/);
  ...
  assert.match(page, /\{copy\.endingReplay\}/);
  ...
  assert.match(css, /animation:ending-replay-in \.45s 4\.25s/);
  assert.match(page, /setCooldown\(10\)/);
  assert.doesNotMatch(page, /setCooldown\(5\)/);
  ...
  assert.match(page, /const APP_VERSION = "v0\.1001"/);
