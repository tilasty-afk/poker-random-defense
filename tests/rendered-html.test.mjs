...
test("현재 전투·연출·모바일 규칙을 고정한다", async () => {
  ...
  assert.match(page, /const ALCHEMY_POOL_RADIUS = 10/);
  assert.match(page, /const MAGE_BLAST_RADIUS = RANGE_PER_CELL \* 2\.5/);
  assert.match(page, /straight:\s*\{[\s\S]*?base:\s*\[46\.8,\s*RANGE_PER_CELL \* 6,\s*\.5\]/);
  assert.match(page, /flush:\s*\{[\s\S]*?base:\s*\[110,\s*RANGE_PER_CELL \* 2,\s*\.5\]/);
  assert.match(page, /fourKind:\s*\{[\s\S]*?base:\s*\[71\.775,\s*RANGE_PER_CELL \* 3,\s*1\.15\]/);
  ...
  assert.match(page, /case "high": return "적 처치 시 1G 추가 · 다른 징집병 1기당 공격력 100% 증가"/);
  assert.match(page, /damage: priestDamageBuff \+ conscriptStacks, speed:/);
  assert.match(i18n, /category === "high" \? source\.replace\("33%", "100%"\)/);
  ...
  assert.match(page, /case "straight": return "장거리 · 치명타 확률 50% · 치명타 피해 5배 · 보스 추가피해 50%"/);
  assert.match(page, /case "pair": return "2초간 받는 피해 100% 추가 표식을 남긴다"/);
  assert.match(page, /tower\.category === "pair"[^}]+markExpirations\.set\(target\.id, now \+ 2000\)/);
  assert.match(page, /markedMultiplier = markExpirations\.has\(id\) \? 2 : 1/);
  assert.match(page, /baseDamage \* \(critical \? 5 : 1\) \* \(target\.boss \? 1\.5 : 1\)/);
  assert.match(page, /baseDamage \* \(target\.boss \? 2 : 1\)/);
  ...
  assert.match(page, /rawAttackInterval = 500 \/ Math\.max/);
  assert.match(page, /towerId: tower\.id,\s*expiresAt: now \+ 320/);
  assert.match(page, /attackFx\.some\(fx => fx\.towerId === tower\.id\) \? "attacking"/);
  assert.match(page, /fixedUtility = best\.category === "triple" \|\| best\.category === "flush" \|\| best\.category === "fullHouse"/);
  assert.match(page, /triple:\s*\{[\s\S]*?base:\s*\[257\.4,\s*RANGE_PER_CELL \* 4,\s*\.1\]/);
  assert.match(page, /flush:\s*\{[\s\S]*?base:\s*\[110,\s*RANGE_PER_CELL \* 2,\s*\.5\]/);
  assert.match(page, /fullHouse:\s*\{[\s\S]*?base:\s*\[65\.76,\s*RANGE_PER_CELL \* 4,\s*\.65\]/);
  assert.match(page, /className="inventory-selection"/);
  assert.match(page, /selectedInventoryUnit\.effect === "returner" \? "강화 비용 -60%"/);
  assert.match(page, /className="field-unit-actions"/);
  assert.match(page, /function sellTower\(\)/);
  assert.doesNotMatch(page, /className=\{`hand-result summon-preview \$\{gameStarted/);
  assert.match(page, /className=\{`unit-dock/);
  assert.match(page, /className="hand-glance-stats"/);
  ...
  assert.match(page, /className=\{`court-suit /);
  assert.match(page, />\{SYMBOL\[card\.suit\]\}<\/i>/);
  ...
  assert.match(page, /window\.addEventListener\("keydown", handleGameShortcut\)/);
  assert.match(page, /\/\^Digit\(\[1-7\]\)\$\/\.exec\(event\.code\)/);
  assert.match(page, /event\.code === "Space"/);
  assert.match(page, /event\.code === "KeyR"/);
  assert.match(page, /event\.code === "KeyR"[\s\S]{0,100}redrawAll\(\)/);
  assert.match(page, /event\.code === "KeyD"/);
  assert.match(page, /event\.code === "BracketLeft"/);
  assert.match(page, /event\.code === "BracketRight"/);
  ...
  assert.match(page, /fourKind:[^\n]+base: \[71\.775, RANGE_PER_CELL \* 3, 1\.15\]/);
  ...
  assert.match(page, /\["fourKind", "triplePair", "backStraight", "mountain", "sevenStraight"\]\.includes\(category\)\)\s*return 4/);
  assert.match(page, /\["straightFlush", "royalFlush", "fiveKind", "doubleTriple", "backStraightFlush"\]\.includes\(category\)\)\s*return 5/);
  assert.match(page, /\["fourFullHouse", "sixKind", "sevenKind"\]\.includes\(category\)\)\s*return 6/);
  ...
  assert.match(page, /setGold\(v => v \+ 5000\)/);
  ...
  assert.match(page, /speed: hasFlagBuff\(tower, towers\) \? \.2 : 0/);
  assert.match(page, /high: \[2, 3, 4\]/);
  assert.match(page, /pair: \[3, 5, 7\]/);
  assert.match(page, /twoPair: \[5, 8, 11\]/);
  assert.match(page, /fixedUtility = best\.category === "triple" \|\| best\.category === "flush" \|\| best\.category === "fullHouse"/);
  assert.match(page, /a\.hp - b\.hp \|\| b\.progress - a\.progress/);
  assert.doesNotMatch(page, /attack-\$\{tower\.id\}-\$\{now\}-chain/);
  assert.match(page, /사거리 내 체력이 가장 낮은 적 우선 공격/);
  ...
  assert.match(i18n, /inventory:"兵营"/);
  assert.match(i18n, /inventory:"兵舎"/);
  ...
  assert.match(css, /\.field-unit-actions\{width:min\(360px,94%\);min-height:0/);
  assert.match(css, /\.field-unit-actions>\.command-unit,\.field-unit-actions>\.command-stats,\.field-unit-actions>p\{display:none!important\}/);
  ...
  assert.doesNotMatch(page, /summon-preview \$\{gameStarted \? "" : "prestart-hidden"\}/);
  assert.match(page, /className="hand-wallet"/);
  assert.match(page, /className="wallet-coin"/);
  assert.doesNotMatch(page, /<small>GOLD :<\/small>/);
  assert.match(page, /className="hand-glance-name"/);
  assert.match(page, /className="hand-glance-copy"/);
  assert.match(page, /className="settings-overlay"/);
  assert.match(page, /\[bgmOn, setBgmOn\] = useState\(false\)/);
  assert.match(page, /className=\{bgmOn \? "on" : "off"\}/);
  assert.match(page, /className="settings-restart" onClick=\{restart\}/);
  assert.match(page, /className="guide-grid job-guide" tabIndex=\{0\} onWheel=/);
  ...
  assert.match(page, /legendary = \["fourKind", "triplePair", "backStraight", "mountain", "sevenStraight"\]\.includes/);
  assert.match(page, /transcendent = \["straightFlush", "royalFlush", "fiveKind", "doubleTriple", "backStraightFlush"\]\.includes/);
  assert.match(page, /tierLabel = legendary \? T\.legend : transcendent \? T\.transcendent/);
  assert.match(page, /\["fourKind", "triplePair", "backStraight", "mountain", "sevenStraight"\]\.includes\(category\)/);
  assert.match(page, /\["straightFlush", "royalFlush", "fiveKind", "doubleTriple", "backStraightFlush"\]\.includes\(category\)/);
  assert.match(page, /\["fourFullHouse", "sixKind", "sevenKind"\]\.includes\(category\)/);
  ...
  assert.match(css, /--desktop-board:min\(730px,calc\(100dvh - 360px\)\)/);
  assert.match(css, /\.battle-wrap\{overflow:hidden\}/);
  assert.match(css, /\.card-slot:nth-child\(1\) \.poker-card::after\{content:"1"\}/);
  assert.match(css, /\.actions>button:nth-child\(3\)::after\{content:"R"\}/);
  assert.match(css, /\.actions \.primary-action::after\{content:"SPACE"\}/);
  assert.match(css, /\.upgrade-row:not\(\.speed-row\) button::after\{content:"\["\}/);
  assert.match(css, /\.speed-row button::after\{content:"\]"\}/);
  assert.match(css, /\.field-unit-actions button\.sell::after,\.inventory-selection>button::after\{content:"D"/);
  assert.match(css, /content:"JOKER";font-size:10px/);
  assert.match(page, /const UNIT_GLYPH/);
  ...
  assert.match(css, /speed-row button::before\{content:"⌛"\}/);
  ...
  assert.match(page, /if \(jokers\.length === 3\)/);
  assert.match(page, /function sevenCardSpecials\(cards: Card\[\]\)/);
  assert.match(page, /category = mountain \? "royalFlush" : back \? "backStraightFlush" : "straightFlush"/);
  assert.match(page, /const sevenWindows = \[\[14, 2, 3, 4, 5, 6, 7\], \.\.\.Array\.from\(\{ length: 7 \}/);
  assert.match(page, /allowed && unique\.size === normal\.length && window\.filter\(rank => !unique\.has\(rank\)\)\.length === jokers\) add\("sevenStraight"/);
  assert.match(page, /countA <= 3 && countB <= 3 && missing <= jokers\) add\("doubleTriple"/);
  assert.match(page, /saintessReady = inventory\.filter\(unit => unit\.effect === "purge"\)\.length >= 5/);
  assert.match(page, /setDirectEndingActive\(false\); setReturnerEndingActive\(false\); setSaintessEndingActive\(true\)/);
  assert.match(page, /const HEART_THEME_UNIT: Partial<Record<Category, string>>/);
  assert.match(page, /\[heartTheme, setHeartTheme\] = useState\(false\)/);
  assert.match(page, /function startHeartMode\(\)/);
  assert.match(page, /className="heart-start" onClick=\{startHeartMode\}/);
  assert.match(page, /sprites\/units\/themes\/beautiful-women/);
  assert.match(page, /setGameStarted\(false\); setHeartTheme\(false\)/);
  assert.match(css, /\.actions \.heart-start\{/);
  assert.match(page, /add\("sevenKind", rank\)/);
  ...
  assert.match(css, /priest-buffed::after\{content:"✦"/);
  ...
  assert.match(page, /const APP_VERSION = "v0\.3006"/);
  assert.match(css, /tier-dot\.tier-4/);
  assert.match(css, /tier-dot\.tier-5/);
  assert.match(css, /tier-dot\.tier-6/);
  assert.match(css, /#37ff78/);
  ...
  assert.match(page, /showMonsterImages \? "IMG ON" : "IMG OFF"/);
  assert.match(css, /html\.enemy-images-off \.enemy \.enemy-face::after\{content:var\(--enemy-wave\)/);
  assert.match(css, /\.monster-image-control\{display:none;z-index:29\}/);
  assert.match(css, /\.monster-image-control\{display:block;position:fixed;top:calc\(env\(safe-area-inset-top\) \+ 125px\)/);
