...
test("현재 전투·연출·모바일 규칙을 고정한다", async () => {
  ...
  assert.match(page, /straight:\s*\{[\s\S]*?base:\s*\[46\.8,\s*RANGE_PER_CELL \* 5,\s*\.5\]/);
  assert.match(page, /flush:\s*\{[\s\S]*?base:\s*\[110,\s*RANGE_PER_CELL \* 2,\s*\.5\]/);
  assert.match(page, /fourKind:\s*\{[\s\S]*?base:\s*\[65\.25,\s*RANGE_PER_CELL \* 6,\s*1\.15\]/);
  ...
  assert.match(page, /case "straight": return "치명타 확률 50%, 치명타 피해 5배, 보스 추가피해 50%"/);
  ...
  assert.match(page, /fourKind:[^\n]+base: \[65\.25, RANGE_PER_CELL \* 6, 1\.15\]/);
  ...
  assert.match(page, /case "straightFlush": return "매우 강한 단일 피해"/);
  ...
  assert.match(page, /사제 1기당 모든 아군 공격력 50% 증가/);
  ...
  assert.match(page, /const priestStacks = priestBuffStacks\(tower, towers\), conscriptStacks = conscriptBuffStacks\(tower, towers\)/);
  assert.match(page, /return \{ damage: priestStacks \* \.5 \+ conscriptStacks, speed: 0 \}/);
  ...
  assert.match(page, /const APP_VERSION = "v0\.2003"/);
