...
test("현재 전투·연출·모바일 규칙을 고정한다", async () => {
  ...
  assert.match(page, /royalFlush:\s*\{[\s\S]*?base:\s*\[122,\s*100,\s*1\.4\]/);
  ...
  assert.match(page, /straightFlush:[^\n]+base: \[195\.2, RANGE_PER_CELL \* 6, \.732\]/);
  ...
  assert.match(page, /BOSS_HP_DIFFICULTY_STEPS = \[\[20, 2\], \[40, 2\.3\], \[60, 2\.8\], \[80, 3\.5\], \[100, 4\.5\]\]/);
  assert.match(page, /function hpDifficultyForWave\(wave: number, bossWave = false\)/);
  assert.match(page, /hpDifficultyForWave\(wave, bossWave\)/);
  ...
  assert.match(page, /공격 사거리 4칸 · 버프 범위 2칸 내 사제 외 아군 공격력 \+20% · 공격속도 \+20% · 사제 수만큼 중복/);
  assert.match(page, /tower\.category !== "fullHouse" && towers\.some/);
  assert.match(page, /stackCount = towers\.filter\(priest =>/);
  ...
  assert.match(page, /const APP_VERSION = "v0\.1"/);
  assert.match(page, /bossHpUnits: 100/);
