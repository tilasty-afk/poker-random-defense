...
test("현재 전투·연출·모바일 규칙을 고정한다", async () => {
  ...
  assert.match(page, /baseHp: 100, hpScale: 3\.078, normalWaveHpCompensation: 40 \/ 30/);
  ...
  assert.match(page, /공격 사거리 4칸 · 버프 범위 2칸 내 사제 외 아군 공격력 \+50% · 중복 적용 불가/);
  assert.match(page, /tower\.category !== "fullHouse" && towers\.some/);
  assert.match(page, /return \{ damage: inBuffRange \? \.5 : 0, speed: 0 \}/);
