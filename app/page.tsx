...
export default function Home() {
    ...
    return <main className="game-shell"><section className="game-frame" aria-label={term(locale, T.brand)}><header className="topbar"><div className="brand-mark">{SYMBOL.spade}</div><div className="brand-copy"><strong>{term(locale, T.brand)}</strong><span>FORTRESS OF FATE</span></div><div className="wave-chip"><span>WAVE</span><strong>{String(wave).padStart(3, "0")} / 100</strong></div><div className="resources"><span>{"\u2620"} <b>{population}/200</b></span><span>{"\u25CF"} <b>{gold}</b></span><button className="sound-toggle" aria-label={soundOn ? "Sound off" : "Sound on"} onClick={toggleSound}>{soundOn ? "♪" : "×"}</button></div></header>
