...
function monsterTrait(monster: (typeof MONSTERS)[number]) { return monster.hp >= 1.6 ? "중장갑" : monster.speed >= 1.18 ? "고속" : monster.hp >= 1.3 ? "고체력" : monster.speed <= .7 ? "둔중" : "균형"; }
function baseHpForWave(wave: number) { const progress = Math.max(0, (wave - BALANCE.hardRampStart) / (100 - BALANCE.hardRampStart)), hardRamp = 1 + progress * (BALANCE.hardRampMax - 1); return BALANCE.baseHp * (1 + wave * BALANCE.hpPerWave) * Math.pow(BALANCE.hpGrowth, wave - 1) * hardRamp; }
function spawnIntervalForWave(wave: number) { const progress = (wave - 1) / 99; return Math.round(BALANCE.spawnInterval - (BALANCE.spawnInterval - BALANCE.minSpawnInterval) * progress); }
...
function roleDescription(unit: Result) { const tier = (unit.tier || 3) - 1; switch (unit.category) {
    case "high": return "기본 원거리 단일 공격";
    case "pair": return `속사 + 받는 피해 ${[8, 12, 16][tier]}% 표식`;
    case "twoPair": return `투척 도끼 ${[2, 3, 4][tier]}명 연쇄`;
    case "triple": return `반경 ${[8, 11, 14][tier]} 범위 폭발`;
    case "straight": return `장거리 + 피해 20% 표식 + 치명타 ${[15, 25, 35][tier]}%`;
    case "flush": return "독 장판 + 이동속도 50% 감소";
    case "fullHouse": return "범위 35 내 아군 공격 +20% · 속도 +20%";
    case "fourKind": return `강력한 단일 검기 + 보스 ${[60, 90, 120][tier]}% 추가 피해`;
...
export default function Home() {
    ...
    const waveTarget = wave % 10 === 0 ? 1 : 40, isBossWave = wave % 10 === 0, monsterKind = monsterKindForWave(isBossWave ? Math.max(1, wave - 1) : wave), monster = MONSTERS[monsterKind];
        ...
        const timer = window.setInterval(() => setEnemies(current => {
            ...
            for (const tower of towers) {
                ...
                if (tower.category === "pair") {
                    ...
                    add(target.id, baseDamage, "direct", [1.08, 1.12, 1.16][i]);
                    continue;
                }
                ...
                if (tower.category === "straight") {
                    const target = inRange.slice().sort((a, b) => Number(b.boss) - Number(a.boss) || b.hp - a.hp)[0], critical = Math.random() < [.15, .25, .35][i];
                    ...
                    add(target.id, baseDamage * (critical ? 2 : 1));
                    if (critical)
                ...
                if (tower.category === "fourKind") {
                    ...
                    const targetPosition = positions.get(target.id)!;
                    attackCandidates[attackCandidates.length - 1] = { ...attackCandidates[attackCandidates.length - 1], tx: targetPosition.x, ty: targetPosition.y };
                    add(target.id, baseDamage * (target.boss ? [1.6, 1.9, 2.2][i] : 1));
                    continue;
                }
    ...
    function toggleSound() { const next = !soundOn; setSoundOn(next); const audio = gameAudioRef.current || createGameAudio(); gameAudioRef.current = audio; audio.setMuted(!next); if (next)
        void audio.unlock().then(unlocked => { if (unlocked)
            audio.play("summon"); }); }
    function toggleRunning() { const next = !running; unlockAudio(); setRunning(next); if (next)
        setMessage("방어전 시작"); }
    ...
    function redraw() { if (cooldown > 0)
        ...
        return;
    } const cost = selected.length === hand.length ? 5 : selected.length * 3; if (gold < cost) {
        setMessage(T.noGold);
        return;
    } const kept = hand.filter(card => !selected.includes(card.id)), replacements = drawFromDeck(selected.length, kept); let replacementIndex = 0; const next = hand.map(card => selected.includes(card.id) ? replacements[replacementIndex++] : card); setGold(v => v - cost); setHand(next); setSelected([]); setMessage(T.rerolled); playSound("reroll"); }
    function recruit() { if (cooldown > 0)
        return; if (!running && !(wave === 1 && spawned === 0 && inventory.length + towers.length === 0)) {
    ...
    const tutorials = [{ title: "1. 세븐포커 소환", body: "7장 중 가장 강한 5장이 자동으로 강조되며 그 족보가 직업을 결정합니다. 흑백·컬러 조커는 와일드 카드입니다." }, { title: "2. 배치·회수·판매", body: "유닛을 눌러 빈 칸에 배치하세요. 필요 없는 유닛은 회수하거나 판매해 리롤 골드를 마련할 수 있습니다. 희귀 유닛은 판매 버튼을 두 번 눌러야 합니다." }, { title: "3. 강화와 전투 효과", body: "공격력은 레벨당 10%, 공격속도는 레벨당 5%씩 제한 없이 상승합니다. 빨간 점멸은 직접 타격, 녹색은 연금술 장판, 보라색은 운명술사의 지속 피해입니다." }, { title: "4. 200 인구를 막아라", body: "적은 탈출하지 않고 계속 순환합니다. 일반 몬스터는 인구 1, 보스는 인구 20입니다. 보스는 게임 시간 2분 안에 처치해야 합니다." }];
    return <main className="game-shell"><section className="game-frame" aria-label={T.brand}><header className="topbar"><div className="brand-mark">{SYMBOL.spade}</div><div className="brand-copy"><strong>{T.brand}</strong><span>FORTRESS OF FATE</span></div><div className="wave-chip"><span>WAVE</span><strong>{String(wave).padStart(3, "0")} / 100</strong></div><div className="resources"><span>{"\u2620"} <b>{population}/200</b></span><span>{"\u25CF"} <b>{gold}</b></span><button className="sound-toggle" aria-label={soundOn ? "효과음 끄기" : "효과음 켜기"} onClick={toggleSound}>{soundOn ? "♪" : "×"}</button></div></header>
    <div className="speed-controls" aria-label="게임 재생 속도">{([1, 1.5, 2, 4] as const).map(speed => <button key={speed} className={gameSpeed === speed ? "active" : ""} aria-pressed={gameSpeed === speed} onClick={() => setGameSpeed(speed)}>{speed}×</button>)}<output aria-live="polite" className={activeBoss || bossWaveHold > 0 ? "boss-time" : ""}>{activeBoss && bossTimeLeft !== null ? `BOSS ${formatTimer(bossTimeLeft)}${bossWaveHold > 0 ? ` · NEXT ${formatTimer(bossWaveHold)}` : ""}` : bossWaveHold > 0 ? `NEXT ${formatTimer(bossWaveHold)}` : `${spawned}/${waveTarget} · ${population}/200`}</output></div>
    <div className="battle-wrap"><div className="battle-header"><div><span className="status-dot"/> {isBossWave ? "BOSS WAVE" : monster[0]} <small>{isBossWave ? `RANK ${wave / 10}` : monsterTrait(monster)}</small></div><strong>{spawned}/{waveTarget} · {population}/200</strong></div><div className="battlefield"><div className="forest-noise"/><div className="loop-segment loop-top"/><div className="loop-segment loop-right"/><div className="loop-segment loop-bottom"/><div className="loop-segment loop-left"/><div className="inner-ground"><span>DEFENSE<br />ZONE</span></div><div className="spawn-gate">SPAWN</div>{activeBoss && <div className="boss-health"><span>RANK {activeBoss.bossRank} · {activeBoss.name}</span><i><b style={{ width: `${Math.max(0, activeBoss.hp / activeBoss.maxHp) * 100}%` }}/></i><strong>{Math.ceil(activeBoss.hp).toLocaleString()}</strong></div>}{SLOTS.map((slot, index) => { const tower = towers.find(t => t.slot === index), blocked = isPathSlot(index), targetState = !blocked && (selectedInventory || selectedTower) && tower?.id !== selectedTower ? (tower ? "swap-target" : "move-target") : ""; return <button type="button" key={index} disabled={blocked} aria-label={blocked ? "몬스터 이동로" : tower ? `${tower.job} 배치칸` : `${index + 1}번 빈 배치칸`} onClick={() => useSlot(index)} className={`grid-slot ${blocked ? "blocked" : ""} ${tower ? "tower-slot occupied" : ""} ${tower?.effect || ""} ${tower?.category === "fullHouse" ? "priest-aura" : ""} ${tower && isPriestBuffed(tower) ? "priest-buffed" : ""} ${tower?.id === selectedTower ? "selected" : ""} ${tower && attackFx.some(fx => fx.towerId === tower.id) ? "attacking" : ""} ${targetState}`} style={{ left: `${slot.x}%`, top: `${slot.y}%` }}>{tower ? <><img src={attackFx.some(fx => fx.towerId === tower.id) ? tower.image.replace("/units/", "/units/attack/") : `${tower.image}?idle=1`} alt=""/><span className={`tier-dot tier-${tower.tier || 3}`}>{tower.tierLabel}</span></> : !blocked && <span className="slot-plus">+</span>}</button>; })}{selectedPlaced && <div className="range-preview" style={{ left: `${SLOTS[selectedPlaced.slot].x}%`, top: `${SLOTS[selectedPlaced.slot].y}%`, width: `${selectedPlaced.range * 2}%` }}/>}{alchemyPools.map(pool => <span key={pool.id} className="alchemy-pool" style={{ left: `${pool.x}%`, top: `${pool.y}%`, width: `${pool.radius * 2}%` }}/>)}{attackFx.map(fx => { const dx = fx.tx - fx.x, dy = fx.ty - fx.y, length = Math.hypot(dx, dy), angle = Math.atan2(dy, dx) * 180 / Math.PI; return <span key={fx.id} className={`attack-fx attack-${fx.category}`}><i className="attack-trace" style={{ left: `${fx.x}%`, top: `${fx.y}%`, width: `${length}%`, transform: `rotate(${angle}deg)` }}/><b className="attack-impact" style={{ left: `${fx.tx}%`, top: `${fx.ty}%`, width: `${fx.radius * 2}%` }}/></span>; })}{enemies.map(e => { const p = pointOnPath(e.progress); return <div key={e.id} title={`${e.boss ? "보스 · " : ""}${e.name} · HP ${Math.ceil(e.hp)}`} className={`enemy ${e.boss ? `boss boss-rank-${e.bossRank}` : ""} ${e.marked ? "marked" : ""} ${e.slowed ? "slowed" : ""} ${e.cursed ? "cursed" : ""} ${e.hitPulse ? "hit-flash-a" : "hit-flash-b"}`} style={{ left: `${p.x}%`, top: `${p.y}%`, backgroundColor: e.color }}><span className="enemy-face">{e.boss ? "B" : String(e.kind + 1).padStart(2, "0")}</span>{e.boss && <b className="boss-badge">BOSS</b>}{e.marked && <b className="status-mark">!</b>}{e.slowed && <b className="status-slow">*</b>}{e.cursed && <b className="status-curse">◆</b>}<i style={{ width: `${Math.max(0, e.hp / e.maxHp) * 100}%` }}/></div>; })}{hitFx.map(fx => <span key={fx.id} className={`hit-number ${fx.critical ? "critical" : ""}`} style={{ left: `${fx.x}%`, top: `${fx.y}%` }}>{fx.critical ? "CRIT " : "-"}{fx.amount}</span>)}{waveCue && <div className={`wave-cue ${isBossWave ? "boss-cue" : ""}`}>{waveCue}<small>{isBossWave ? "강적 1체 · 일반 몬스터 80마리분" : "40 ENEMIES INCOMING"}</small></div>}{!selectedPlaced && <div className="battle-message" aria-live="polite">{cooldown > 0 ? `NEXT HAND ${cooldown}s` : selectedInventory ? "배치할 칸을 선택하세요" : message}</div>}{selectedPlaced && <aside className="field-unit-actions" aria-label={`${selectedPlaced.job} 조작창`}><button className="command-close" onClick={() => setSelectedTower(null)} aria-label="유닛 선택 해제">×</button><div className="command-unit"><img src={selectedPlaced.image} alt=""/><span><small>{selectedPlaced.tierLabel} · {selectedPlaced.slot + 1}번 칸</small><strong>{selectedPlaced.job}</strong>{isPriestBuffed(selectedPlaced) && <em>사제 버프 적용 중</em>}</span></div><div className="command-stats"><span><small>공격</small><b>{Math.round(selectedPlaced.damage * attackMultiplier * 10) / 10}</b></span><span><small>사거리</small><b>{selectedPlaced.range}</b></span><span><small>속도</small><b>{(selectedPlaced.speed * attackSpeedMultiplier).toFixed(2)}</b></span></div><p>빛나는 칸을 눌러 이동 · 유닛 칸은 위치 교체</p><div className="command-actions"><button onClick={recallTower}>인벤토리 회수</button><button className="sell" onClick={sellTower}>판매 <b>+{sellValue(selectedPlaced)}G</b></button></div></aside>}</div></div>
    <section className={`unit-inventory ${selectedInventoryUnit ? "has-selection" : ""}`}><div className="inventory-title"><strong>내 유닛 인벤토리</strong><span>{inventory.length}</span></div><div className="inventory-list">{inventory.length === 0 ? <p>소환한 유닛이 여기에 보관됩니다.</p> : inventory.map(unit => <button key={unit.id} onClick={() => chooseInventory(unit.id)} className={selectedInventory === unit.id ? "active" : ""}><img src={unit.image} alt=""/><span>{unit.job}</span><small>{unit.effect === "purge" ? "사용" : unit.tierLabel}</small></button>)}</div>{selectedInventoryUnit && <div className="inventory-selection"><img src={selectedInventoryUnit.image} alt=""/><div><strong>{selectedInventoryUnit.job}</strong><small>{selectedInventoryUnit.tierLabel} · 판매가 {sellValue(selectedInventoryUnit)}G</small></div><span>빈 칸을 눌러 배치</span><button onClick={sellInventoryUnit} aria-label={`${selectedInventoryUnit.job} ${sellValue(selectedInventoryUnit)}골드에 판매`}>판매 <b>+{sellValue(selectedInventoryUnit)}G</b></button></div>}<div className="army-upgrade"><div className="upgrade-row"><div><span>전체 공격력</span><strong>LV.{attackLevel} · +{attackLevel * 10}%</strong></div><button disabled={gold < upgradeCost} onClick={upgradeAttack}>강화 <b>{upgradeCost}G</b></button></div><div className="upgrade-row speed-row"><div><span>전체 공격속도</span><strong>LV.{attackSpeedLevel} · +{attackSpeedLevel * 5}%</strong></div><button disabled={gold < speedUpgradeCost} onClick={upgradeAttackSpeed}>강화 <b>{speedUpgradeCost}G</b></button></div></div></section>
    <section className="hand-result summon-preview"><div className="summon-portrait"><img src={result.image} alt=""/><i>SUMMON</i></div><div className="summon-copy"><div className="summon-path"><span>현재 족보</span><b>{result.label}</b><em>→</em><span>등장 유닛</span></div><strong><i className={`tier-badge tier-${result.tier || 3}`}>{result.tierLabel}</i> {result.job}</strong><small>{T.attack} {result.damage} / {T.range} {result.range} / {T.speed} {result.speed}</small><small className="effect-note">{roleDescription(result)}</small></div></section>
    <section className="hand-panel"><div className="section-title"><div><span>{T.hand}</span><small>{cooldown > 0 ? `NEXT HAND ${cooldown}s` : "7장 중 가장 강한 조합이 자동 선택됩니다"}</small></div><div className="hand-meta"><b>{result.label}</b><span className="deck-count">52 + 2J</span></div></div><div className={`cards ${cooldown > 0 ? "cooling" : ""}`}>{hand.map(card => <button key={card.id} disabled={cooldown > 0} onClick={() => toggle(card.id)} className={`poker-card ${result.bestCardIds.includes(card.id) ? "best" : ""} ${selected.includes(card.id) ? "active" : ""}`} aria-pressed={selected.includes(card.id)} aria-label={card.rank === 15 ? `${card.joker === "color" ? "컬러" : "흑백"} 조커` : undefined}><span className={`${card.suit === "heart" || card.suit === "diamond" ? "red-suit" : ""} ${card.joker ? `joker-${card.joker}` : ""}`}>{card.rank === 15 ? (card.joker === "color" ? "C-J" : "B-J") : rankLabel(card.rank)}<small>{card.rank === 15 ? "★" : SYMBOL[card.suit]}</small></span><b>{card.rank === 15 ? (card.joker === "color" ? "COLOR" : "MONO") : SYMBOL[card.suit]}</b><em>{selected.includes(card.id) ? "CHANGE" : result.bestCardIds.includes(card.id) ? "BEST" : "KEEP"}</em></button>)}</div><div className="actions"><button className="primary-action" disabled={cooldown > 0} onClick={recruit}>{cooldown > 0 ? `${cooldown}s` : T.recruit} <b>FREE</b></button><button disabled={cooldown > 0} onClick={redraw}>{"\u21BB"} {T.redraw} <b>{selected.length === hand.length ? 5 : Math.max(3, selected.length * 3)}</b></button><button className={running ? "pause" : "start"} onClick={toggleRunning}>{running ? `II ${T.pause}` : `\u25B6 ${T.start}`}</button></div></section>
    <details className="poker-guide"><summary>{T.guide}<span>{T.open}</span></summary><div className="guide-grid job-guide">{guide.map(([category, job]) => <span key={category}><b>{JOBS[category].label}</b><small>{job}{category !== "royalFlush" && category !== "fiveKind" && category !== "sixKind" ? ` / ${T.beginner} ${T.middle} ${T.elite}` : ""}</small></span>)}</div><button className="tutorial-reopen" onClick={() => setTutorialStep(0)}>처음 설명 다시 보기</button></details>
