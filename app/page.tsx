...
export default function Home() {
    ...
    const [gameSpeed, setGameSpeed] = useState<1 | 2 | 4 | 8>(1), [bossTimeLeft, setBossTimeLeft] = useState<number | null>(null), [bossWaveHold, setBossWaveHold] = useState(0), gameClockRef = useRef(0), lastTickAtRef = useRef(0), bossDeadlineRef = useRef(0), bossWaveReleaseRef = useRef(0);
    ...
    <div className="speed-controls" aria-label="게임 재생 속도">{([1, 2, 4, 8] as const).map(speed => <button key={speed} className={gameSpeed === speed ? "active" : ""} aria-pressed={gameSpeed === speed} onClick={() => setGameSpeed(speed)}>{speed}×</button>)}<output aria-live="polite" className={activeBoss || bossWaveHold > 0 ? "boss-time" : ""}>{activeBoss && bossTimeLeft !== null ? `BOSS ${formatTimer(bossTimeLeft)}${bossWaveHold > 0 ? ` · NEXT ${formatTimer(bossWaveHold)}` : ""}` : bossWaveHold > 0 ? `NEXT ${formatTimer(bossWaveHold)}` : `${spawned}/${waveTarget} · ${population}/200`}</output></div>
