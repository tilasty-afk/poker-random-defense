...
function tierFor(category: Category, power: number): Tier | null { if (["fourKind", "triplePair", "backStraight", "mountain", "sevenStraight"].includes(category))
    return 4; if (["straightFlush", "royalFlush", "fiveKind", "doubleTriple", "backStraightFlush"].includes(category))
    return 5; if (["fourFullHouse", "sixKind", "sevenKind"].includes(category))
    return 6; const first = category === "high" ? 8 : 6, second = category === "high" ? 11 : 10; return power <= first ? 1 : power <= second ? 2 : 3; }
...
function roleDescription(unit: Result, locale: Locale = activeLocale) { if (locale !== "ko")
    ...
    case "high": return "적 처치 시 1G 추가 · 다른 징집병 1기당 공격력 100% 증가";
...
export default function Home() {
    ...
    useEffect(() => {
        ...
        const timer = window.setInterval(() => setEnemies(current => {
            ...
            const supportBuff = (tower: Tower) => {
                ...
                return { damage: priestDamageBuff + conscriptStacks, speed: hasFlagBuff(tower, towers) ? .2 : 0 };
            };
    ...
    <details className="poker-guide" data-locale={locale}><summary>{copy.guide}<span>{copy.open}</span></summary><div className="guide-grid job-guide" tabIndex={0} onWheel={event => event.stopPropagation()}>{guide.map(([category, job]) => <span key={category}><b>{term(locale, JOBS[category].label)}</b><small>{term(locale, job)}{["fourKind", "triplePair", "backStraight", "mountain", "sevenStraight"].includes(category) ? ` / ${term(locale, T.legend)}` : ["straightFlush", "royalFlush", "fiveKind", "doubleTriple", "backStraightFlush"].includes(category) ? ` / ${term(locale, T.transcendent)}` : ["fourFullHouse", "sixKind", "sevenKind"].includes(category) ? ` / ${term(locale, T.unique)}` : ` / ${term(locale, T.beginner)} ${term(locale, T.middle)} ${term(locale, T.elite)}`}</small></span>)}</div><button className="tutorial-reopen" onClick={() => setTutorialStep(0)}>{copy.tutorialAgain}</button></details>
