...
export default function Home() {
    ...
    const [endingActive, setEndingActive] = useState(false), [saintessEndingActive, setSaintessEndingActive] = useState(false), [directEndingActive, setDirectEndingActive] = useState(false), [hiddenIntroActive, setHiddenIntroActive] = useState(false), [gameStarted, setGameStarted] = useState(false);
    activeLocale = locale;
    ...
    useEffect(() => { if (!running || spawned < waveTarget || wave >= MAX_WAVE || hiddenIntroActive || (isBossWave && bossWaveHold > 0 && wave !== VISIBLE_MAX_WAVE) || (wave === VISIBLE_MAX_WAVE && enemies.length > 0))
        return; if (wave === VISIBLE_MAX_WAVE) { setHiddenIntroActive(true); setMessage("???"); return; } setWave(v => v + 1); setSpawned(0); setMessage(`WAVE ${wave + 1}`); }, [running, spawned, wave, waveTarget, isBossWave, bossWaveHold, enemies.length, hiddenIntroActive]);
    ...
    useEffect(() => { if (!running)
        return; setWaveCue(isHiddenWave ? "???" : isBossWave ? `BOSS WAVE ${wave}` : `WAVE ${wave}`); const timer = window.setTimeout(() => setWaveCue(null), 1400 / gameSpeed); return () => window.clearTimeout(timer); }, [wave, running, isBossWave, isHiddenWave, gameSpeed]);
    ...
    useEffect(() => { if (!running || wave !== HIDDEN_WAVE || spawned < waveTarget || enemies.length > 0 || saintessEndingActive)
        return; setWon(true); setRunning(false); setPlaybackPaused(true); setDirectEndingActive(true); setMessage("ABSOLUTE TRIUMPH"); }, [running, wave, spawned, waveTarget, enemies.length, saintessEndingActive]);
    ...
    <details className="poker-guide" data-locale={locale}><summary>{copy.guide}<span>{copy.open}</span></summary><div className="guide-grid job-guide">{guide.map(([category, job]) => <span key={category}><b>{term(locale, JOBS[category].label)}</b><small>{term(locale, job)}{category === "fourKind" || category === "straightFlush" ? ` / ${term(locale, T.elite)}` : category !== "royalFlush" && category !== "fiveKind" && category !== "sixKind" && category !== "sevenKind" ? ` / ${term(locale, T.beginner)} ${term(locale, T.middle)} ${term(locale, T.elite)}` : ""}</small></span>)}</div><button className="tutorial-reopen" onClick={() => setTutorialStep(0)}>{copy.tutorialAgain}</button></details>
    ...
    <HiddenWaveIntro active={hiddenIntroActive} className="hidden-wave-intro"/>
