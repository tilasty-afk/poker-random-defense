...
export default function Home() {
    ...
    const [gameSpeed, setGameSpeed] = useState<1 | 2 | 4 | 8>(1), [playbackPaused, setPlaybackPaused] = useState(false), [bossTimeLeft, setBossTimeLeft] = useState<number | null>(null), [bossWaveHold, setBossWaveHold] = useState(0), gameClockRef = useRef(0), lastTickAtRef = useRef(0), bossDeadlineRef = useRef(0), bossWaveReleaseRef = useRef(0);
    ...
    } setGameSpeed(gameSpeed === 1 ? 2 : gameSpeed === 2 ? 4 : 8); }
    ...
    <div className="playback-control"><button className={playbackPaused ? "paused" : ""} aria-label={playbackPaused ? "일시정지 해제하고 1배속으로 재생" : `현재 ${gameSpeed}배속, 다음 단계로 변경`} onClick={cyclePlayback}>{playbackPaused ? "Ⅱ" : `${gameSpeed}×`}</button><output aria-live="polite" className={activeBoss || bossWaveHold > 0 ? "boss-time" : ""}>{activeBoss && bossTimeLeft !== null ? `BOSS ${formatTimer(bossTimeLeft)}${bossWaveHold > 0 ? ` · NEXT ${formatTimer(bossWaveHold)}` : ""}` : bossWaveHold > 0 ? `NEXT ${formatTimer(bossWaveHold)}` : `${spawned}/${waveTarget} · ${population}/200`}</output></div>
