...
const BOSS_HP_BY_WAVE: Record<number, number> = { 10: 45000, 20: 80000, 30: 140000, 40: 220000, 50: 340000, 60: 500000, 70: 720000, 80: 1000000, 90: 1550000, 100: 2600000 };
...
function hpDifficultyForWave(wave: number) { return NORMAL_HP_DIFFICULTY_STEPS.find(([endWave]) => wave <= endWave)?.[1] ?? NORMAL_HP_DIFFICULTY_STEPS[NORMAL_HP_DIFFICULTY_STEPS.length - 1][1]; }
function baseHpForWave(wave: number) { return BALANCE.baseHp * (1 + wave * BALANCE.hpPerWave) * Math.pow(BALANCE.hpGrowth, wave - 1) * hpDifficultyForWave(wave); }
function bossHpForWave(wave: number) { return BOSS_HP_BY_WAVE[wave] ?? BOSS_HP_BY_WAVE[100]; }
...
function roleDescription(unit: Result, locale: Locale = activeLocale) { if (locale !== "ko")
    ...
    case "fullHouse": return "공격 사거리 4칸 · 버프 범위 3칸 내 사제 외 아군 공격력 +20% · 공격속도 +20% · 사제 수만큼 중복";
...
export default function Home() {
    ...
    const [endingActive, setEndingActive] = useState(false), [gameStarted, setGameStarted] = useState(false);
    activeLocale = locale;
