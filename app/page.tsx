...
const T_BASE = {
    ...
    beginner: "\uCD08\uAE09", middle: "\uC911\uAE09", elite: "\uC815\uC608", unique: "\uCD08\uC6D4",
...
export default function Home() {
    ...
    <details className="poker-guide" data-locale={locale}><summary>{copy.guide}<span>{copy.open}</span></summary><div className="guide-grid job-guide" tabIndex={0} onWheel={event => event.stopPropagation()}>{guide.map(([category, job]) => <span key={category}><b>{term(locale, JOBS[category].label)}</b><small>{term(locale, job)}{category === "fourKind" || category === "straightFlush" || category === "royalFlush" || category === "fiveKind" ? ` / ${term(locale, T.unique)}` : category !== "sixKind" && category !== "sevenKind" ? ` / ${term(locale, T.beginner)} ${term(locale, T.middle)} ${term(locale, T.elite)}` : ""}</small></span>)}</div><button className="tutorial-reopen" onClick={() => setTutorialStep(0)}>{copy.tutorialAgain}</button></details>
