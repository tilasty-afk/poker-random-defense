...
export function roleCopy(locale: Locale, category: string, tier: number) {
  ...
  const values: Record<string, [number[], string, string, string]> = {
    ...
    flush:[[],"Poison pool + 50% movement slow","毒池 + 移速降低50%","毒沼 + 移動速度50%低下"], fullHouse:[[],"4-tile attack range; non-Priest allies within 2 tiles gain +50% ATK (does not stack)","攻击射程4格；2格内非祭司友军攻击力+50%（不可叠加）","攻撃射程4マス・2マス内の司祭以外の味方の攻撃力+50%（重複不可）"],
